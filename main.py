...
def process_hospital(hospital, start):
    ...
    cost = try4distance(start, end)
    if cost == "no path found":
        return [0.0, -1.0, thresholds]
    
    cost = float(cost)
    RW = hospital["R"] * gussian_w(cost, thresholds)
    return [0.0 if RW == 0 else RW, cost, thresholds]
...
def process_grid_i(i, grid, length):
    ...
    for handle in result_handles:
        RW, cost, thresholds = ray.get(handle)
        grid["acc"] += RW

        if thresholds == 60:
            if grid["nearest_3"] == -1 or grid["nearest_3"] > cost:
                grid["nearest_3"] = cost
        elif thresholds == 45:
            if grid["nearest_2"] == -1 or grid["nearest_2"] > cost:
                grid["nearest_2"] = cost
        elif thresholds == 30:
            if grid["nearest_1"] == -1 or grid["nearest_1"] > cost:
                grid["nearest_1"] = cost

    if i % 10000 == 0:
        print_and_log(f"S2, {i}/{length}")

    return f"{i},{grid['acc']},{grid['nearest_3']},{grid['nearest_2']},{grid['nearest_1']}\n"
...
# process a region
def process_region(region_name, shape):
    # --- init ---
    global grids
    global grids_id
    global hospitals
    ...
    check_and_create("./data/tmp/acc.csv", "index,acc,nearest_3,nearest_2,nearest_1\n")
    ...
    # --- Step 1 ---
    time1 = time.time()
    if not os.path.exists(f"./data/tmp/S1Rj_reindex.csv"):
        ...
        print_and_log(f"{region_name} S1Rj_reindex already exists")
    # --- Step 1 ---
    ...
    # --- clean ---
    check_and_remove(f"./data/tmp/acc_reindex.csv")
    check_and_remove(f"./data/tmp/S1Rj_reindex.csv")
...
for I, R in china.iterrows():
    ...
    try:
        ray.init()
        process_region(rn, gpd.GeoDataFrame([R]))
    finally:
        p.terminate()
        p.wait()
